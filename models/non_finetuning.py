...
class NonFineTuning(object):
    def __init__(self, Xtrain, Ytrain, gpusave=False, summary=True, summaryout=False,
        ...
        if self.modelname == 'original':
            self.model = self.__original_forward()
        else:
            self.model = self.__premodel_forward()
    ...
    def __original_forward(self):
        ...
        hid = Dropout(0.3)(hid)
        hid = Conv2D(64,(5,5),padding='same',activation='relu')(inputs)
        ...
        model = Model(inputs, outputs, name='non-finetuning_model')
        if self.summary:
            self.__summary(model, self.summaryout)

        return model

    def __premodel_forward(self):
        if self.modelname == 'vgg16':
            premodel = VGG16(weights=None, include_top=False,
                    input_shape=self.Xtrain.shape[1:])
        elif self.modelname == 'vgg19':
            premodel = VGG19(weights=None, include_top=False,
                    input_shape=self.Xtrain.shape[1:])
        else:
            raise ValueError('Incorrect model name.')

        fc_inputs = Input(shape=premodel.output_shape[1:])
        ...
        top_model = Model(fc_inputs, fc_outputs)
        total_model = Model(premodel.inputs, top_model(premodel.outputs),
                name='{}_non-finetuning_model'.format(self.modelname))
        if self.summary:
